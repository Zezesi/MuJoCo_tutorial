...
class quadcopter:
  ...
  def update_angle_conrol(self):
    ...
    desPitch = self.pid_v_x(curVel[0]) # desired change of velocity in x direction is achieved by the change of the pitch angle
    desRoll = -self.pid_v_y(curVel[1]) # desired change of velocity in y direction is achieved by the change of the roll angle
  ...
  def update_motor_control(self):
    ...
    cmd_thrust = self.pid_v_z(curVel[2]) + 3.2495 # desired change of velocity in z direction is achieved by the change of the thrust force
